...
public class CatchBulbasaur extends JFrame{
	private Player player;
	public CatchBulbasaur(Player player) {
		this.player = player;
		setSize(1360,800);
		setTitle("BAG");
		setVisible(true);
        ...
        usePokeBall.addActionListener(new ActionListener() {
        	public void actionPerformed(ActionEvent e) {
        		if(player.getBallBag().get(0).getValue() > 0){
        			...
        			if(gotit == random) {
        				player.getPokeBag().add(new Bulbasaur("Wild Bulbasaur"));
        				setVisible(false);
        		...
        		setVisible(false);
        		CatchBulbasaur ree = new CatchBulbasaur(player);
        	}
        });
