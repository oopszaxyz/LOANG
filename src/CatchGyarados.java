...
public class CatchGyarados extends JFrame{
	private Player player;
	public CatchGyarados(Player player) {
		this.player = player;
		setSize(1360,800);
		setTitle("BAG");
		setVisible(true);
        ...
        usePokeBall.addActionListener(new ActionListener() {
        	public void actionPerformed(ActionEvent e) {
        		if(player.getBallBag().get(0).getValue() > 0){
        			...
        			if(gotit == random) {
        				player.getPokeBag().add(new Gyarados("Wild Gyarados"));
        				setVisible(false);
        		...
        		setVisible(false);
        		CatchGyarados ree = new CatchGyarados(player);
        	}
        });
