...
public class CatchCharmander extends JFrame{
	private Player player;
	public CatchCharmander(Player player) {
		this.player = player;
		setSize(1360,800);
		setTitle("BAG");
		setVisible(true);
        ...
        usePokeBall.addActionListener(new ActionListener() {
        	public void actionPerformed(ActionEvent e) {
        		if(player.getBallBag().get(0).getValue() > 0){
        			...
        			if(gotit == random) {
        				player.getPokeBag().add(new Charmander("Wild Charmander"));
        				setVisible(false);
        		...
        		setVisible(false);
        		CatchCharmander ree = new CatchCharmander(player);
        	}
        });
