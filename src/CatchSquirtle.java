...
public class CatchSquirtle extends JFrame{
	private Player player;
	public CatchSquirtle(Player player) {
		this.player = player;
		setSize(1360,800);
		setTitle("BAG");
		setVisible(true);
        ...
        usePokeBall.addActionListener(new ActionListener() {
        	public void actionPerformed(ActionEvent e) {
        		if(player.getBallBag().get(0).getValue() > 0){
        			...
        			if(gotit == random) {
        				player.getPokeBag().add(new Squirtle("Wild Squirtle"));
        				setVisible(false);
        		...
        		setVisible(false);
        		CatchSquirtle ree = new CatchSquirtle(player);
        	}
        });
